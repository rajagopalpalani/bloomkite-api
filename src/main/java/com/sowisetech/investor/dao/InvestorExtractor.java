...
public class InvestorExtractor implements ResultSetExtractor<Investor> {

	String deleteflag;

	public InvestorExtractor(String delete_flag) {
		deleteflag = delete_flag;
	}

	@Override
	public Investor extractData(ResultSet rs) throws SQLException, DataAccessException {
		...
		while (rs.next()) {
			...
			if (type.equals("invInt") && delete_flag != null && delete_flag.equals(deleteflag)) {
				if (invInterestList == null || invInterestList.size() == 0) {

...
public class AdvisorDaoImpl implements AdvisorDao {
	...
	public int advSignup(Advisor advisor, String encryptPass, int promoCount) {
		try {
			...
			String sql = "INSERT INTO `advisor` (`advId`,`name`, `emailId`,`panNumber`, `password`,`userName`, `phoneNumber`,`partyStatusId`,`created`,`updated`,`delete_flag`,`advType`,`parentPartyId`,`corporateLable`,`workFlowStatus`,`created_by`,`updated_by`,`promoCount`) values (?,AES_ENCRYPT(?, ?),AES_ENCRYPT(LOWER(?), ?),?,?,AES_ENCRYPT(LOWER(?), ?),?,?, ?, ?, ?, ?, ?, ?, ?,?,?,?)";
			int result = jdbcTemplate.update(sql, advisor.getAdvId(), advisor.getName(), encryptPass,
					advisor.getEmailId(), encryptPass, advisor.getPanNumber(), advisor.getPassword(),
					advisor.getUserName(), encryptPass, advisor.getPhoneNumber(),
	...
	public int addParty(Advisor advisor, String encryptPass) {
		try {
			...
			String sqlForPartyInsert = "INSERT INTO `party` (`partyStatusId`,`roleBasedId`,`emailId`,`password`,`panNumber`,`phoneNumber`,`userName`,`created`,`updated`,`delete_flag`,`parentPartyId`,`created_by`,`updated_by`) values (?,?,AES_ENCRYPT(LOWER(?), ?), ?,?,?, AES_ENCRYPT(LOWER(?), ?), ?, ?,?,?,?,?)";
			int result = jdbcTemplate.update(sqlForPartyInsert, advisor.getPartyStatusId(), advisor.getAdvId(),
					advisor.getEmailId(), encryptPass, advisor.getPassword(), advisor.getPanNumber(),
					advisor.getPhoneNumber(), advisor.getUserName(), encryptPass, timestamp, timestamp,
	...
	public int update(String advId, Advisor advisor, String encryptPass) {
		try {
			...
			String sql1 = "UPDATE `advisor` SET `name`= AES_ENCRYPT(?, ?), `emailId`=AES_ENCRYPT(?, ?), `phoneNumber`=AES_ENCRYPT(?, ?),`userName`=AES_ENCRYPT(?, ?),`designation`=?,`displayName`=?,`dob`=AES_ENCRYPT(?, ?),`gender`=?,`panNumber`=AES_ENCRYPT(?, ?),`address1`=?,`address2`=?,`state`=?,`city`=?,`aboutme`=?, `pincode`=?, `partyStatusId`=? ,`updated`=?,`updated_by`=?, `firmType`=?, `parentPartyId`=?, `corporateLable`=?,`website`=?,`imagePath`=?,`gst`=? WHERE `advId`=?";
	...
	public Advisor fetchAdvisorByEmailId(String emailId, String deleteflag, String encryptPass) {
		try {
			String sql = "SELECT `advId`, `advType`, AES_DECRYPT(`name`,?) name, AES_DECRYPT(`userName`,?) userName, `designation`, AES_DECRYPT(`emailId`,?) emailId, `password`, AES_DECRYPT(`phoneNumber`,?) phoneNumber, `delete_flag`, `partyStatusId`, `created`, `updated`,`created_by`,`updated_by`,`displayName`, AES_DECRYPT(`dob`,?) dob, `gender`, AES_DECRYPT(`panNumber`,?) panNumber, `address1`, `address2`, `state`, `city`, `pincode`, `aboutme`, `firmType`, `parentPartyId`, `corporateLable`, `website` FROM `advisor` WHERE AES_DECRYPT(`emailId`,?) = ? AND `delete_flag`=?";
	...
	public int addAdvPersonalInfo(String advId, Advisor adv, String encryptPass) {
		try {
			...
			String sqlUpdate = "UPDATE `advisor` SET `username`=AES_ENCRYPT(?,?),`name`=AES_ENCRYPT(?,?),`panNumber`=AES_ENCRYPT(?,?),`displayName`=?,`firmType`=?,`corporateLable`=?,`website`=?,`imagePath`=?,`dob`=AES_ENCRYPT(?,?),`designation`=?,`gender`=?,`address1`=?,`address2`=?,`state`=?,`city`=?,`pincode`=?,`aboutme`=?,`updated`=?,`updated_by`=? WHERE `advId`=?";
	...
	public Advisor checkEmailAvailability(String emailId, String encryptPass) {
		try {
			String sql = "SELECT `advId`, `advType`, AES_DECRYPT(`name`,?) name,AES_DECRYPT(`userName`,?) userName, `designation`, AES_DECRYPT(`emailId`,?) emailId, `password`, AES_DECRYPT(`phoneNumber`,?) phoneNumber, `delete_flag`, `partyStatusId`, `created`, `updated`, `created_by`, `updated_by`, `displayName`, AES_DECRYPT(`dob`,?) dob, `gender`, AES_DECRYPT(`panNumber`,?) panNumber, `address1`, `address2`, `state`, `city`, `pincode`, `aboutme` FROM `advisor` WHERE AES_DECRYPT(`emailId`,) = ?";
			RowMapper<Advisor> rowMapper = new BeanPropertyRowMapper<Advisor>(Advisor.class);
			return jdbcTemplate.queryForObject(sql, rowMapper, encryptPass, encryptPass, encryptPass, encryptPass,
	...
	public int addInv(Investor investor, String deleteflag, String encryptPass) {
		try {
			...
			ZonedDateTime zdt = ZonedDateTime.now(ZoneId.of("Asia/Calcutta"));
			Timestamp timestamp = Timestamp.valueOf(zdt.toLocalDateTime());
			String sql = "INSERT INTO `investor` (`invId`,`fullName`, `displayName`, `dob`, `gender`, `emailId`, `password`, `phoneNumber`, `pincode`, `partyStatusId`, `created`, `updated`,`delete_flag`,`created_by`,`updated_by`) values (?,AES_ENCRYPT(?,?),?,AES_ENCRYPT(?,?),?,AES_ENCRYPT(LOWER(?),?), ?, ?, ?, ?, ?, ?, ?,?,?)";
			int result = jdbcTemplate.update(sql, investor.getInvId(), investor.getFullName(), encryptPass,
					...
					investor.getEmailId(), encryptPass, investor.getPassword(),
					investor.getPhoneNumber(), investor.getPincode(), investor.getPartyStatusId(),
	...
	public int addPartyInv(Investor investor, String deleteflag, String encryptPass) {
		try {
			...
			ZonedDateTime zdt = ZonedDateTime.now(ZoneId.of("Asia/Calcutta"));
			Timestamp timestamp = Timestamp.valueOf(zdt.toLocalDateTime());
			String sqlForPartyInsert = "INSERT INTO `party` (`partyStatusId`,`roleBasedId`,`emailId`,`password`,`userName`,`phoneNumber`,`created`,`updated`,`delete_flag`,`created_by`,`updated_by`) values (?,?,AES_ENCRYPT(LOWER(?),?),?,AES_ENCRYPT(LOWER(?),?), ?, ?,?,?,?,?)";
			int result = jdbcTemplate.update(sqlForPartyInsert, investor.getPartyStatusId(), investor.getInvId(),
					...
					investor.getPhoneNumber(), timestamp, timestamp, deleteflag, investor.getCreated_by(),
	...
	public int addKeyPeople(KeyPeople keyPeople, String deleteflag, String encryptPass) {
		try {
			...
			ZonedDateTime zdt = ZonedDateTime.now(ZoneId.of("Asia/Calcutta"));
			Timestamp timestamp = Timestamp.valueOf(zdt.toLocalDateTime());
			String sql = "INSERT INTO `keypeople` (`fullName`,`designation`, `image`,`parentPartyId`, `created`, `updated`,`delete_flag`,`created_by`,`updated_by`) values (AES_ENCRYPT(?,?),?,?,?,?,?,?,?,?)";
	...
	public List<KeyPeople> fetchKeyPeopleByParentId(long parentPartyId, String deleteflag, String encryptPass) {
		try {
			String sql = "SELECT `keyPeopleId`, AES_DECRYPT(`fullName`,?) fullName, `designation`, `image`, `parentPartyId`,`created`,`updated`,`delete_flag`,`created_by`,`updated_by` FROM `keypeople` WHERE `parentPartyId` = ? AND `delete_flag`=?";
	...
	public int addSigninVerification(String emailId, String encryptPass) {
		...
		Timestamp timestamp = Timestamp.valueOf(zdt.toLocalDateTime());
		try {
			String sqlForPartyInsert = "INSERT INTO `signin_verification` (`mailId`,`created`) values (AES_ENCRYPT(LOWER(?),?), ?)";
	...
	public Party fetchPartyByPhoneNumber(String phoneNumber, String encryptPass) {
		try {
			String sql = "SELECT `partyId`,`partyStatusId`,`roleBasedId`,`parentPartyId`,AES_DECRYPT(`emailId`,?) emailId,`password`,AES_DECRYPT(`userName`,?) userName,AES_DECRYPT(`phoneNumber`,?) phoneNumber,AES_DECRYPT(`panNumber`,?) panNumber,`created`,`updated`,`delete_flag`,`created_by`,`updated_by` FROM `party` WHERE AES_DECRYPT(`phoneNumber`,?)=?";
	...
	public Party fetchPartyByPAN(String panNumber, String encryptPass) {
		try {
			String sql = "SELECT `partyId`,`partyStatusId`,`roleBasedId`,`parentPartyId`,AES_DECRYPT(`emailId`,?) emailId,`password`,AES_DECRYPT(`userName`,?) userName,AES_DECRYPT(`phoneNumber`,?) phoneNumber,AES_DECRYPT(`panNumber`,?) panNumber,`created`,`updated`,`delete_flag`,`created_by`,`updated_by` FROM `party` WHERE AES_DECRYPT(`panNumber`,?)= ?";
			RowMapper<Party> partyMapper = new BeanPropertyRowMapper<Party>(Party.class);
			return jdbcTemplate.queryForObject(sql, partyMapper, encryptPass, encryptPass, encryptPass, encryptPass,
	...
	public Party fetchPartyByUserName(String userName, String encryptPass) {
		try {
			String sql = "SELECT `partyId`,`partyStatusId`,`roleBasedId`,`parentPartyId`,AES_DECRYPT(`emailId`,?) emailId,`password`,AES_DECRYPT(`userName`,?) userName,AES_DECRYPT(`phoneNumber`,?) phoneNumber,AES_DECRYPT(`panNumber`,?) panNumber,`created`,`updated`,`delete_flag`,`created_by`,`updated_by` FROM `party` WHERE AES_DECRYPT(`userName`,?)=?";
			RowMapper<Party> partyMapper = new BeanPropertyRowMapper<Party>(Party.class);
			return jdbcTemplate.queryForObject(sql, partyMapper, encryptPass, encryptPass, encryptPass, encryptPass,
	...
	public int addPublicAdvisor(Advisor adv, String encryptPass) {
		try {
			...
			ZonedDateTime zdt = ZonedDateTime.now(ZoneId.of("Asia/Calcutta"));
			Timestamp timestamp = Timestamp.valueOf(zdt.toLocalDateTime());
			String sql = "INSERT INTO `public_advisor` (`advId`,`name`, `emailId`,`panNumber`, `password`,`userName`, `phoneNumber`,`partyStatusId`,`created`,`updated`,`created_by`,`updated_by`,`delete_flag`,`advType`,`parentPartyId`,`corporateLable`,`workFlowStatus`) values (?,AES_ENCRYPT(?, ?),AES_ENCRYPT(?, ?),AES_ENCRYPT(?, ?),?,AES_ENCRYPT(?, ?),AES_ENCRYPT(?, ?),?, ?, ?, ?, ?, ?,?,?,?,?)";
	...
	public int addPublicAdvPersonalInfo(String advId, Advisor adv, String encryptPass) {
		try {
			ZonedDateTime zdt = ZonedDateTime.now(ZoneId.of("Asia/Calcutta"));
			Timestamp timestamp = Timestamp.valueOf(zdt.toLocalDateTime());
			String sqlUpdate = "UPDATE `public_advisor` SET `displayName`=?,`firmType`=?,`corporateLable`=?,`website`=?,`imagePath`=?,`dob`=AES_ENCRYPT(?,?),`designation`=?,`gender`=?,`address1`=?,`address2`=?,`state`=?,`city`=?,`pincode`=?,`aboutme`=?,`isVerified`=?,`isMobileVerified`=?,`updated`=?,`updated_by`=? WHERE `advId`=?";
	...
	public KeyPeople fetchKeyPeopleByKeyPeopleId(long keyPeopleId, String deleteflag, String encryptPass) {
		try {
			String sql = "SELECT `keyPeopleId`,AES_DECRYPT(`fullName`,?) fullName,`designation`,`image`,`parentPartyId`,`created`,`updated`,`delete_flag`,`created_by`,`updated_by` FROM `keypeople` WHERE `delete_flag`=? AND `keyPeopleId`=?";
	...
	public int modifyKeyPeople(long keyPeopleId, KeyPeople keyPeople, String encryptPass) {
		try {
			...
			ZonedDateTime zdt = ZonedDateTime.now(ZoneId.of("Asia/Calcutta"));
			Timestamp timestamp = Timestamp.valueOf(zdt.toLocalDateTime());
			String sql1 = "UPDATE `keypeople` SET `fullName`= AES_ENCRYPT(?, ?), `designation` =?, `image`=?,`parentPartyId`=?,`updated`=?,`updated_by`=? WHERE `keyPeopleId`=?";
	...
	// advType,AES_DECRYPT(adv.`name`,?) name,adv.`designation`
	// designation,AES_DECRYPT(adv.`emailId`,?) emailId,adv.`password`
	// password,AES_DECRYPT(adv.`userName`,?) userName,AES_DECRYPT(adv.`phoneNumber`,?)
	...
	// displayName,AES_DECRYPT(adv.`dob`,?) dob,adv.`gender`
	// gender,AES_DECRYPT(adv.`panNumber`,?) panNumber,adv.`address1`
	...
	// try {
	// String sql = "SELECT * FROM (SELECT inv.`invId`
	// invId,AES_DECRYPT(inv.`fullName`,?) fullName,inv.`displayName`
	// displayName,AES_DECRYPT(inv.`dob`,?) dob,AES_DECRYPT(inv.`emailId`,?)
	// emailId,inv.`gender` gender,inv.`password` password,AES_DECRYPT(inv.`userName`,?)
	// userName,AES_DECRYPT(inv.`phoneNumber`,?) phoneNumber,inv.`pincode`
	...
	public List<Advisor> fetchExploreAdvisorList(Pageable pageable, String sortByState, String sortByCity,
		...
		try {
			String sql = "SELECT * FROM (SELECT adv.`advId` advId,adv.`advType` advType,AES_DECRYPT(adv.`name`,?) name,adv.`designation` designation,AES_DECRYPT(adv.`emailId`,?) emailId,adv.`password` password,AES_DECRYPT(adv.`userName`,?) userName,AES_DECRYPT(adv.`phoneNumber`,?) phoneNumber,adv.`delete_flag` delete_flag,adv.`partyStatusId` partyStatusId,adv.`created` created,adv.`updated` updated,adv.`created_by` created_by,adv.`updated_by` updated_by,adv.`displayName` displayName,AES_DECRYPT(adv.`dob`,?) dob,adv.`gender` gender,AES_DECRYPT(adv.`panNumber`,?) panNumber,adv.`address1` address1,adv.`address2` address2,adv.`state` state,adv.`city` city,adv.`pincode` pincode,adv.`aboutme` aboutme,adv.`firmType` firmType,adv.`corporateLable` corporateLable,adv.`website` website,adv.`parentPartyId` parentPartyId,adv.`imagePath` imagePath,adv.`isVerified` isVerified,adv.`verifiedBy` verifiedBy,adv.`verified` verified,adv.`isMobileVerified` isMobileVerified,adv.`workFlowStatus` workFlowStatus,adv.`approvedDate` approvedDate,adv.`approvedBy` approvedBy,adv.`revokedDate` revokedDate,adv.`revokedBy` revokedBy, exp.expId COL_A, exp.company COL_B,  exp.designation COL_C,exp.location COL_D,exp.fromYear COL_E,exp.toYear COL_F,exp.advId COL_ADVID,exp.delete_flag COL_DELETEFLAG, '' COL_I,'' COL_J,'' COL_K,'' COL_L,'' COL_M,'' COL_N, 'exp' VALUE FROM (SELECT * FROM public_advisor WHERE delete_flag =? AND advId != ? AND `state` LIKE '%"
					...
					+ "%' AND `displayName` LIKE '%" + sortByDisplayName + "%' ORDER BY AES_DECRYPT(name,?) ASC LIMIT  "
					+ pageable.getPageSize() + " OFFSET " + pageable.getOffset()
					+ ") adv LEFT JOIN public_experience exp ON (adv.advId = exp.advId) \r\n"
	...
	public int updatePersonalInfoInParty(String emailId, String phoneNumber, String panNumber, String userName,
		...
		try {
			...
			String sql = "UPDATE `party` SET `emailId`=AES_ENCRYPT(?,?),`phoneNumber`=AES_ENCRYPT(?,?), `panNumber`=AES_ENCRYPT(?,?), `userName`=AES_ENCRYPT(?,?), `updated`=?,`updated_by`=? WHERE `roleBasedId`=?";
	...
	public List<Advisor> fetchSearchAdvisorList(Pageable pageable, String panNumber, String emailId, String phoneNumber,
		...
		try {
			...
			String sql = "SELECT * FROM (SELECT adv.`advId` advId,adv.`advType` advType,AES_DECRYPT(adv.`name`,?) name,adv.`designation` designation,AES_DECRYPT(adv.`emailId`,?) emailId,adv.`password` password,AES_DECRYPT(adv.`userName`,?) userName,AES_DECRYPT(adv.`phoneNumber`,?) phoneNumber,adv.`delete_flag` delete_flag,adv.`partyStatusId` partyStatusId,adv.`created` created,adv.`updated` updated,adv.`created_by` created_by,adv.`updated_by` updated_by,adv.`displayName` displayName,AES_DECRYPT(adv.`dob`,?) dob,adv.`gender` gender,AES_DECRYPT(adv.`panNumber`,?) panNumber,adv.`address1` address1,adv.`address2` address2,adv.`state` state,adv.`city` city,adv.`pincode` pincode,adv.`aboutme` aboutme,adv.`firmType` firmType,adv.`corporateLable` corporateLable,adv.`website` website,adv.`parentPartyId` parentPartyId,adv.`imagePath` imagePath,adv.`isVerified` isVerified,adv.`verifiedBy` verifiedBy,adv.`verified` verified,adv.`isMobileVerified` isMobileVerified,adv.`workFlowStatus` workFlowStatus,adv.`approvedDate` approvedDate,adv.`approvedBy` approvedBy,adv.`revokedDate` revokedDate,adv.`revokedBy` revokedBy, exp.expId COL_A, exp.company COL_B,  exp.designation COL_C,exp.location COL_D,exp.fromYear COL_E,exp.toYear COL_F,exp.advId COL_ADVID,exp.delete_flag COL_DELETEFLAG, '' COL_I,'' COL_J,'' COL_K,'' COL_L,'' COL_M,'' COL_N, 'exp' VALUE FROM (SELECT * FROM advisor WHERE delete_flag =? AND AES_DECRYPT(`panNumber`,?) LIKE '%"
					+ panNumber + "%' AND AES_DECRYPT(`emailId`,?) LIKE '%" + emailId
					+ "%' AND AES_DECRYPT(`phoneNumber`,?) LIKE '%" + phoneNumber + "%' AND `workFlowStatus` LIKE '%"
					+ workFlowStatusId + "%' AND `advType` LIKE '%" + advType + "%' ORDER BY created ASC LIMIT "
					+ pageable.getPageSize() + " OFFSET " + pageable.getOffset()
					+ " ) adv LEFT JOIN experience exp ON (adv.advId = exp.advId) \r\n"
					+ "UNION ALL SELECT adv.`advId` advId,adv.`advType` advType,AES_DECRYPT(adv.`name`,?) name,adv.`designation` designation,AES_DECRYPT(adv.`emailId`,?) emailId,adv.`password` password,AES_DECRYPT(adv.`userName`,?) userName,AES_DECRYPT(adv.`phoneNumber`,?) phoneNumber,adv.`delete_flag` delete_flag,adv.`partyStatusId` partyStatusId,adv.`created` created,adv.`updated` updated,adv.`created_by` created_by,adv.`updated_by` updated_by,adv.`displayName` displayName,AES_DECRYPT(adv.`dob`,?) dob,adv.`gender` gender,AES_DECRYPT(adv.`panNumber`,?) panNumber,adv.`address1` address1,adv.`address2` address2,adv.`state` state,adv.`city` city,adv.`pincode` pincode,adv.`aboutme` aboutme,adv.`firmType` firmType,adv.`corporateLable` corporateLable,adv.`website` website,adv.`parentPartyId` parentPartyId,adv.`imagePath` imagePath,adv.`isVerified` isVerified,adv.`verifiedBy` verifiedBy,adv.`verified` verified,adv.`isMobileVerified` isMobileVerified,adv.`workFlowStatus` workFlowStatus,adv.`approvedDate` approvedDate,adv.`approvedBy` approvedBy,adv.`revokedDate` revokedDate,adv.`revokedBy` revokedBy, edu.eduId,edu.degree,edu.field,edu.institution,edu.fromYear,edu.toYear,edu.advId,edu.delete_flag,'','','','','','', 'edu' VALUE FROM (SELECT * FROM advisor  WHERE delete_flag = ? AND AES_DECRYPT(`panNumber`,?) LIKE '%"
					+ panNumber + "%' AND AES_DECRYPT(`emailId`,?) LIKE '%" + emailId
					+ "%' AND AES_DECRYPT(`phoneNumber`,?) LIKE '%" + phoneNumber + "%' AND `workFlowStatus` LIKE '%"
					+ workFlowStatusId + "%' AND `advType` LIKE '%" + advType + "%' ORDER BY created ASC LIMIT "
					+ pageable.getPageSize() + " OFFSET " + pageable.getOffset()
					+ ") adv LEFT JOIN education edu ON (adv.advId = edu.advId) \r\n"
					+ "UNION ALL SELECT adv.`advId` advId,adv.`advType` advType,AES_DECRYPT(adv.`name`,?) name,adv.`designation` designation,AES_DECRYPT(adv.`emailId`,?) emailId,adv.`password` password,AES_DECRYPT(adv.`userName`,?) userName,AES_DECRYPT(adv.`phoneNumber`,?) phoneNumber,adv.`delete_flag` delete_flag,adv.`partyStatusId` partyStatusId,adv.`created` created,adv.`updated` updated,adv.`created_by` created_by,adv.`updated_by` updated_by,adv.`displayName` displayName,AES_DECRYPT(adv.`dob`,?) dob,adv.`gender` gender,AES_DECRYPT(adv.`panNumber`,?) panNumber,adv.`address1` address1,adv.`address2` address2,adv.`state` state,adv.`city` city,adv.`pincode` pincode,adv.`aboutme` aboutme,adv.`firmType` firmType,adv.`corporateLable` corporateLable,adv.`website` website,adv.`parentPartyId` parentPartyId,adv.`imagePath` imagePath,adv.`isVerified` isVerified,adv.`verifiedBy` verifiedBy,adv.`verified` verified,adv.`isMobileVerified` isMobileVerified,adv.`workFlowStatus` workFlowStatus,adv.`approvedDate` approvedDate,adv.`approvedBy` approvedBy,adv.`revokedDate` revokedDate,adv.`revokedBy` revokedBy, awd.awardId, awd.imagePath,awd.issuedBy,awd.title,awd.year,'',awd.advId,awd.delete_flag,'','','','','','','awd' VALUE FROM (SELECT * FROM advisor  WHERE delete_flag = ? AND AES_DECRYPT(`panNumber`,?) LIKE '%"
					+ panNumber + "%' AND AES_DECRYPT(`emailId`,?) LIKE '%" + emailId
					+ "%' AND AES_DECRYPT(`phoneNumber`,?) LIKE '%" + phoneNumber + "%' AND `workFlowStatus` LIKE '%"
					+ workFlowStatusId + "%' AND `advType` LIKE '%" + advType + "%' ORDER BY created ASC LIMIT "
					+ pageable.getPageSize() + " OFFSET " + pageable.getOffset()
					+ ") adv LEFT JOIN award awd ON (adv.advId = awd.advId) \r\n"
					+ "UNION ALL SELECT adv.`advId` advId,adv.`advType` advType,AES_DECRYPT(adv.`name`,?) name,adv.`designation` designation,AES_DECRYPT(adv.`emailId`,?) emailId,adv.`password` password,AES_DECRYPT(adv.`userName`,?) userName,AES_DECRYPT(adv.`phoneNumber`,?) phoneNumber,adv.`delete_flag` delete_flag,adv.`partyStatusId` partyStatusId,adv.`created` created,adv.`updated` updated,adv.`created_by` created_by,adv.`updated_by` updated_by,adv.`displayName` displayName,AES_DECRYPT(adv.`dob`,?) dob,adv.`gender` gender,AES_DECRYPT(adv.`panNumber`,?) panNumber,adv.`address1` address1,adv.`address2` address2,adv.`state` state,adv.`city` city,adv.`pincode` pincode,adv.`aboutme` aboutme,adv.`firmType` firmType,adv.`corporateLable` corporateLable,adv.`website` website,adv.`parentPartyId` parentPartyId,adv.`imagePath` imagePath,adv.`isVerified` isVerified,adv.`verifiedBy` verifiedBy,adv.`verified` verified,adv.`isMobileVerified` isMobileVerified,adv.`workFlowStatus` workFlowStatus,adv.`approvedDate` approvedDate,adv.`approvedBy` approvedBy,adv.`revokedDate` revokedDate,adv.`revokedBy` revokedBy, cert.certificateId, cert.imagePath,cert.issuedBy,cert.title,cert.year,'',cert.advId,cert.delete_flag,'' ,'','','','','','cert' VALUE FROM (SELECT * FROM advisor WHERE delete_flag =? AND AES_DECRYPT(`panNumber`,?) LIKE '%"
					+ panNumber + "%' AND AES_DECRYPT(`emailId`,?) LIKE '%" + emailId
					+ "%' AND AES_DECRYPT(`phoneNumber`,?) LIKE '%" + phoneNumber + "%' AND `workFlowStatus` LIKE '%"
					+ workFlowStatusId + "%' AND `advType` LIKE '%" + advType + "%' ORDER BY created ASC LIMIT "
					+ pageable.getPageSize() + " OFFSET " + pageable.getOffset()
					+ ") adv LEFT JOIN certificate cert ON (adv.advId = cert.advId) \r\n"
					+ "UNION ALL SELECT adv.`advId` advId,adv.`advType` advType,AES_DECRYPT(adv.`name`,?) name,adv.`designation` designation,AES_DECRYPT(adv.`emailId`,?) emailId,adv.`password` password,AES_DECRYPT(adv.`userName`,?) userName,AES_DECRYPT(adv.`phoneNumber`,?) phoneNumber,adv.`delete_flag` delete_flag,adv.`partyStatusId` partyStatusId,adv.`created` created,adv.`updated` updated,adv.`created_by` created_by,adv.`updated_by` updated_by,adv.`displayName` displayName,AES_DECRYPT(adv.`dob`,?) dob,adv.`gender` gender,AES_DECRYPT(adv.`panNumber`,?) panNumber,adv.`address1` address1,adv.`address2` address2,adv.`state` state,adv.`city` city,adv.`pincode` pincode,adv.`aboutme` aboutme,adv.`firmType` firmType,adv.`corporateLable` corporateLable,adv.`website` website,adv.`parentPartyId` parentPartyId,adv.`imagePath` imagePath,adv.`isVerified` isVerified,adv.`verifiedBy` verifiedBy,adv.`verified` verified,adv.`isMobileVerified` isMobileVerified,adv.`workFlowStatus` workFlowStatus,adv.`approvedDate` approvedDate,adv.`approvedBy` approvedBy,adv.`revokedDate` revokedDate,adv.`revokedBy` revokedBy, prod.advProdId,prod.licImage, prod.prodId,prod.serviceId,prod.remId,prod.licId,prod.advId,prod.delete_flag,prod.licNumber,prod.validity,prod.created,prod.updated,prod.created_by,prod.updated_by,'prod' VALUE FROM (SELECT * FROM advisor  WHERE delete_flag = ? AND AES_DECRYPT(`panNumber`,?) LIKE '%"
					+ panNumber + "%' AND AES_DECRYPT(`emailId`,?) LIKE '%" + emailId
					+ "%' AND AES_DECRYPT(`phoneNumber`,?) LIKE '%" + phoneNumber + "%' AND `workFlowStatus` LIKE '%"
					+ workFlowStatusId + "%' AND `advType` LIKE '%" + advType + "%' ORDER BY created ASC LIMIT "
					+ pageable.getPageSize() + " OFFSET " + pageable.getOffset()
					+ ") adv LEFT JOIN advproduct prod ON (adv.advId = prod.advId) \r\n"
					+ "UNION ALL SELECT adv.`advId` advId,adv.`advType` advType,AES_DECRYPT(adv.`name`,?) name,adv.`designation` designation,AES_DECRYPT(adv.`emailId`,?) emailId,adv.`password` password,AES_DECRYPT(adv.`userName`,?) userName,AES_DECRYPT(adv.`phoneNumber`,?) phoneNumber,adv.`delete_flag` delete_flag,adv.`partyStatusId` partyStatusId,adv.`created` created,adv.`updated` updated,adv.`created_by` created_by,adv.`updated_by` updated_by,adv.`displayName` displayName,AES_DECRYPT(adv.`dob`,?) dob,adv.`gender` gender,AES_DECRYPT(adv.`panNumber`,?) panNumber,adv.`address1` address1,adv.`address2` address2,adv.`state` state,adv.`city` city,adv.`pincode` pincode,adv.`aboutme` aboutme,adv.`firmType` firmType,adv.`corporateLable` corporateLable,adv.`website` website,adv.`parentPartyId` parentPartyId,adv.`imagePath` imagePath,adv.`isVerified` isVerified,adv.`verifiedBy` verifiedBy,adv.`verified` verified,adv.`isMobileVerified` isMobileVerified,adv.`workFlowStatus` workFlowStatus,adv.`approvedDate` approvedDate,adv.`approvedBy` approvedBy,adv.`revokedDate` revokedDate,adv.`revokedBy` revokedBy, promo.promotionId,promo.title,promo.video,promo.imagePath,promo.aboutVideo,'',promo.advId,promo.delete_flag,'','','','','','','promo' VALUE FROM (SELECT * FROM advisor WHERE delete_flag = ? AND AES_DECRYPT(`panNumber`,?) LIKE '%"
					+ panNumber + "%' AND AES_DECRYPT(`emailId`,?) LIKE '%" + emailId
					+ "%' AND AES_DECRYPT(`phoneNumber`,?) LIKE '%" + phoneNumber + "%' AND `workFlowStatus` LIKE '%"
					+ workFlowStatusId + "%' AND `advType` LIKE '%" + advType + "%' ORDER BY created ASC LIMIT "
					+ pageable.getPageSize() + " OFFSET " + pageable.getOffset()
					+ ") adv LEFT JOIN promotion promo ON (adv.advId = promo.advId) \r\n"
					+ "UNION ALL SELECT adv.`advId` advId,adv.`advType` advType,AES_DECRYPT(adv.`name`,?) name,adv.`designation` designation,AES_DECRYPT(adv.`emailId`,?) emailId,adv.`password` password,AES_DECRYPT(adv.`userName`,?) userName,AES_DECRYPT(adv.`phoneNumber`,?) phoneNumber,adv.`delete_flag` delete_flag,adv.`partyStatusId` partyStatusId,adv.`created` created,adv.`updated` updated,adv.`created_by` created_by,adv.`updated_by` updated_by,adv.`displayName` displayName,AES_DECRYPT(adv.`dob`,?) dob,adv.`gender` gender,AES_DECRYPT(adv.`panNumber`,?) panNumber,adv.`address1` address1,adv.`address2` address2,adv.`state` state,adv.`city` city,adv.`pincode` pincode,adv.`aboutme` aboutme,adv.`firmType` firmType,adv.`corporateLable` corporateLable,adv.`website` website,adv.`parentPartyId` parentPartyId,adv.`imagePath` imagePath,adv.`isVerified` isVerified,adv.`verifiedBy` verifiedBy,adv.`verified` verified,adv.`isMobileVerified` isMobileVerified,adv.`workFlowStatus` workFlowStatus,adv.`approvedDate` approvedDate,adv.`approvedBy` approvedBy,adv.`revokedDate` revokedDate,adv.`revokedBy` revokedBy, brandinfo.advBrandId,brandinfo.prodId,brandinfo.serviceId,brandinfo.brandId,brandinfo.priority,'',brandinfo.advId,brandinfo.delete_flag,'','','','','','', 'brandinfo' VALUE FROM (SELECT * FROM advisor  WHERE delete_flag = ? AND AES_DECRYPT(`panNumber`,?) LIKE '%"
					+ panNumber + "%' AND AES_DECRYPT(`emailId`,?) LIKE '%" + emailId
					+ "%' AND AES_DECRYPT(`phoneNumber`,?) LIKE '%" + phoneNumber + "%' AND `workFlowStatus` LIKE '%"
					+ workFlowStatusId + "%' AND `advType` LIKE '%" + advType + "%' ORDER BY created ASC LIMIT "
					+ pageable.getPageSize() + " OFFSET " + pageable.getOffset()
					+ ") adv LEFT JOIN advbrandinfo brandinfo ON (adv.advId = brandinfo.advId)  \r\n"
					+ "UNION ALL SELECT adv.`advId` advId,adv.`advType` advType,AES_DECRYPT(adv.`name`,?) name,adv.`designation` designation,AES_DECRYPT(adv.`emailId`,?) emailId,adv.`password` password,AES_DECRYPT(adv.`userName`,?) userName,AES_DECRYPT(adv.`phoneNumber`,?) phoneNumber,adv.`delete_flag` delete_flag,adv.`partyStatusId` partyStatusId,adv.`created` created,adv.`updated` updated,adv.`created_by` created_by,adv.`updated_by` updated_by,adv.`displayName` displayName,AES_DECRYPT(adv.`dob`,?) dob,adv.`gender` gender,AES_DECRYPT(adv.`panNumber`,?) panNumber,adv.`address1` address1,adv.`address2` address2,adv.`state` state,adv.`city` city,adv.`pincode` pincode,adv.`aboutme` aboutme,adv.`firmType` firmType,adv.`corporateLable` corporateLable,adv.`website` website,adv.`parentPartyId` parentPartyId,adv.`imagePath` imagePath,adv.`isVerified` isVerified,adv.`verifiedBy` verifiedBy,adv.`verified` verified,adv.`isMobileVerified` isMobileVerified,adv.`workFlowStatus` workFlowStatus,adv.`approvedDate` approvedDate,adv.`approvedBy` approvedBy,adv.`revokedDate` revokedDate,adv.`revokedBy` revokedBy, brandrank.advBrandRankId,brandrank.prodId,brandrank.brandId,brandrank.ranking,'','',brandrank.advId,brandrank.delete_flag,'','','','','','', 'brandrank'VALUE FROM (SELECT * FROM advisor  WHERE delete_flag = ? AND AES_DECRYPT(`panNumber`,?) LIKE '%"
					+ panNumber + "%' AND AES_DECRYPT(`emailId`,?) LIKE '%" + emailId
					+ "%' AND AES_DECRYPT(`phoneNumber`,?) LIKE '%" + phoneNumber + "%' AND `workFlowStatus` LIKE '%"
					+ workFlowStatusId + "%' AND `advType` LIKE '%" + advType + "%' ORDER BY created ASC LIMIT "
					+ pageable.getPageSize() + " OFFSET " + pageable.getOffset()
	...
	public int fetchTotalSearchAdvisorList(String panNumber, String emailId, String phoneNumber, String userName,
		...
		try {
			String sql = "SELECT COUNT(*) FROM `advisor` WHERE delete_flag =? AND AES_DECRYPT(`panNumber`,?) LIKE '%"
					+ panNumber + "%' AND AES_DECRYPT(`emailId`,?) LIKE '%" + emailId
					+ "%' AND AES_DECRYPT(`phoneNumber`,?) LIKE '%" + phoneNumber + "%' AND `workFlowStatus` LIKE '%"
		...
		try {
			...
			if (stateCityPincodeList.size() > 0) {
				String sql = "SELECT * FROM (SELECT adv.`advId` advId,adv.`advType` advType,AES_DECRYPT(adv.`name`,?) name,adv.`designation` designation,AES_DECRYPT(adv.`emailId`,?) emailId,adv.`password` password,AES_DECRYPT(adv.`userName`,?) userName,AES_DECRYPT(adv.`phoneNumber`,?) phoneNumber,adv.`delete_flag` delete_flag,adv.`partyStatusId` partyStatusId,adv.`created` created,adv.`updated` updated,adv.`created_by` created_by,adv.`updated_by` updated_by,adv.`displayName` displayName,AES_DECRYPT(adv.`dob`,?) dob,adv.`gender` gender,AES_DECRYPT(adv.`panNumber`,?) panNumber,adv.`address1` address1,adv.`address2` address2,adv.`state` state,adv.`city` city,adv.`pincode` pincode,adv.`aboutme` aboutme,adv.`firmType` firmType,adv.`corporateLable` corporateLable,adv.`website` website,adv.`parentPartyId` parentPartyId,adv.`imagePath` imagePath,adv.`isVerified` isVerified,adv.`verifiedBy` verifiedBy,adv.`verified` verified,adv.`isMobileVerified` isMobileVerified,adv.`workFlowStatus` workFlowStatus,adv.`approvedDate` approvedDate,adv.`approvedBy` approvedBy,adv.`revokedDate` revokedDate,adv.`revokedBy` revokedBy, exp.expId COL_A, exp.company COL_B,  exp.designation COL_C,exp.location COL_D,exp.fromYear COL_E,exp.toYear COL_F,exp.advId COL_ADVID,exp.delete_flag COL_DELETEFLAG, '' COL_I,'' COL_J,'' COL_K,'' COL_L,'' COL_M,'' COL_N, 'exp' VALUE FROM ((SELECT pub_adv.* FROM public_advisor pub_adv INNER JOIN `public_advbrandinfo` pub_brandinfo ON (pub_adv.`advId` = pub_brandinfo.`advId` AND pub_adv.`delete_flag` = ? AND pub_brandinfo.delete_flag=? AND pub_brandinfo.`prodId` ='"
						+ productId + "' AND pub_brandinfo.`serviceId` ='" + serviceId
						+ "' AND pub_brandinfo.`brandId` = '" + brandId + "') WHERE 1=(CASE WHEN '" + sortByDisplayName
						+ "' = '' THEN 1 ELSE 0 END) AND pub_adv.delete_flag =? ORDER BY AES_DECRYPT(pub_adv.`name`,?) ASC LIMIT "
						+ pageable.getPageSize() + " OFFSET " + pageable.getOffset()
						...
						+ sortByDisplayName
						+ "' IS NOT NULL THEN 1 ELSE 0 END) AND pub_adv.delete_flag =? AND pub_adv.`displayName`= '"
						+ sortByDisplayName + "' ORDER BY AES_DECRYPT(pub_adv.`name`,?) ASC LIMIT " + pageable.getPageSize()
						...
						+ sortByDisplayName
						+ "' IS NOT NULL THEN 1 ELSE 0 END) AND pub_adv.delete_flag =? AND pub_adv.`displayName`= '"
						+ sortByDisplayName + "' ORDER BY AES_DECRYPT(pub_adv.`name`,?) ASC LIMIT	"
						+ pageable.getPageSize() + " OFFSET " + pageable.getOffset()
						+ ")) adv LEFT JOIN public_award awd ON (adv.advId = awd.advId) \r\n"
				...
				sqlAdv = "SELECT * FROM (SELECT adv.`advId` advId,adv.`advType` advType,AES_DECRYPT(adv.`name`,?) name,adv.`designation` designation,AES_DECRYPT(adv.`emailId`,?) emailId,adv.`password` password,AES_DECRYPT(adv.`userName`,?) userName,AES_DECRYPT(adv.`phoneNumber`,?) phoneNumber,adv.`delete_flag` delete_flag,adv.`partyStatusId` partyStatusId,adv.`created` created,adv.`updated` updated,adv.`created_by` created_by,adv.`updated_by` updated_by,adv.`displayName` displayName,AES_DECRYPT(adv.`dob`,?) dob,adv.`gender` gender,AES_DECRYPT(adv.`panNumber`,?) panNumber,adv.`address1` address1,adv.`address2` address2,adv.`state` state,adv.`city` city,adv.`pincode` pincode,adv.`aboutme` aboutme,adv.`firmType` firmType,adv.`corporateLable` corporateLable,adv.`website` website,adv.`parentPartyId` parentPartyId,adv.`imagePath` imagePath,adv.`isVerified` isVerified,adv.`verifiedBy` verifiedBy,adv.`verified` verified,adv.`isMobileVerified` isMobileVerified,adv.`workFlowStatus` workFlowStatus,adv.`approvedDate` approvedDate,adv.`approvedBy` approvedBy,adv.`revokedDate` revokedDate,adv.`revokedBy` revokedBy, exp.expId COL_A, exp.company COL_B,  exp.designation COL_C,exp.location COL_D,exp.fromYear COL_E,exp.toYear COL_F,exp.advId COL_ADVID,exp.delete_flag COL_DELETEFLAG, '' COL_I,'' COL_J,'' COL_K,'' COL_L,'' COL_M,'' COL_N, 'exp' VALUE FROM ((SELECT pub_adv.* FROM public_advisor pub_adv INNER JOIN `public_advbrandinfo` pub_brandinfo ON (pub_adv.`advId` = pub_brandinfo.`advId` AND pub_adv.`delete_flag` = ? AND pub_brandinfo.delete_flag=? AND pub_brandinfo.`prodId` ='"
						+ productId + "' AND pub_brandinfo.`serviceId` ='" + serviceId
						+ "' AND pub_brandinfo.`brandId` = '" + brandId + "') WHERE 1=(CASE WHEN '" + sortByDisplayName
						+ "' = '' THEN 1 ELSE 0 END) AND pub_adv.delete_flag =? ORDER BY AES_DECRYPT(pub_adv.`name`,?) ASC LIMIT "
						+ pageable.getPageSize() + " OFFSET " + pageable.getOffset()
	...
	public Party fetchPartyByPhoneNumberAndDeleteFlag(String phoneNumber, String deleteflag, String encryptPass) {
		try {
			String sql = "SELECT `partyId`,`partyStatusId`,`roleBasedId`,`parentPartyId`,AES_DECRYPT(`emailId`,?) emailId,`password`,AES_DECRYPT(`userName`,?) userName,AES_DECRYPT(`phoneNumber`,?) phoneNumber,AES_DECRYPT(`panNumber`,?) panNumber,`created`,`updated`,`delete_flag`,`created_by`,`updated_by` FROM `party` WHERE AES_DECRYPT(`phoneNumber`,?)=? AND `delete_flag`=?";
			RowMapper<Party> partyMapper = new BeanPropertyRowMapper<Party>(Party.class);
			return jdbcTemplate.queryForObject(sql, partyMapper, encryptPass, encryptPass, encryptPass, encryptPass,
	...
	public int updateUsernameInParty(String advId, Advisor adv, String signedUserId, String encryptPass) {
		try {
			...
			String sql = "UPDATE `party` SET `userName`=AES_ENCRYPT(?,?), `updated`=?,`updated_by`=? WHERE `roleBasedId`=?";
	...
	public int fetchTotalSearchAdvisorListWithEmptyValues(String pan_lc, String email_lc, String phone_lc,
		...
		try {
			String sql = "SELECT COUNT(*) FROM advisor WHERE delete_flag =? AND workFlowStatus LIKE '%"
					+ workFlowStatusId + "%' AND AES_DECRYPT(panNumber,?) LIKE '%" + pan_lc
					+ "%' AND AES_DECRYPT(emailId,?) LIKE '%" + email_lc + "%' AND AES_DECRYPT(phoneNumber,?) LIKE '%" + phone_lc
					+ "%'AND AES_DECRYPT(userName,?) LIKE '%" + userName_lc + "%' OR AES_DECRYPT(userName,?) IS NULL";
	...
	public List<Advisor> fetchSearchAdvisorListWithEmptyValues(Pageable pageable, String pan_lc, String email_lc,
		...
		try {
			...
			String sql = "SELECT * FROM (SELECT adv.`advId` advId,adv.`advType` advType,AES_DECRYPT(adv.`name`,?) name,adv.`designation` designation,AES_DECRYPT(adv.`emailId`,?) emailId,adv.`password` password,AES_DECRYPT(adv.`userName`,?) userName,AES_DECRYPT(adv.`phoneNumber`,?) phoneNumber,adv.`delete_flag` delete_flag,adv.`partyStatusId` partyStatusId,adv.`created` created,adv.`updated` updated,adv.`created_by` created_by,adv.`updated_by` updated_by,adv.`displayName` displayName,AES_DECRYPT(adv.`dob`,?) dob,adv.`gender` gender,AES_DECRYPT(adv.`panNumber`,?) panNumber,adv.`address1` address1,adv.`address2` address2,adv.`state` state,adv.`city` city,adv.`pincode` pincode,adv.`aboutme` aboutme,adv.`firmType` firmType,adv.`corporateLable` corporateLable,adv.`website` website,adv.`parentPartyId` parentPartyId,adv.`imagePath` imagePath,adv.`isVerified` isVerified,adv.`verifiedBy` verifiedBy,adv.`verified` verified,adv.`isMobileVerified` isMobileVerified,adv.`workFlowStatus` workFlowStatus,adv.`approvedDate` approvedDate,adv.`approvedBy` approvedBy,adv.`revokedDate` revokedDate,adv.`revokedBy` revokedBy, exp.expId COL_A, exp.company COL_B,  exp.designation COL_C,exp.location COL_D,exp.fromYear COL_E,exp.toYear COL_F,exp.advId COL_ADVID,exp.delete_flag COL_DELETEFLAG, '' COL_I,'' COL_J,'' COL_K,'' COL_L,'' COL_M,'' COL_N, 'exp' VALUE FROM (SELECT * FROM advisor WHERE delete_flag =?  AND `workFlowStatus` LIKE '%"
					+ workFlowStatusId + "%' AND AES_DECRYPT(`panNumber`,?) LIKE '%" + pan_lc
					+ "%' AND AES_DECRYPT(`emailId`,?) LIKE '%" + email_lc + "%' AND AES_DECRYPT(`phoneNumber`,?) LIKE '%"
					+ phone_lc + "%' AND AES_DECRYPT(`userName`,?) LIKE '%" + userName_lc
					+ "%' OR AES_DECRYPT(userName,?) IS NULL ORDER BY created ASC LIMIT " + pageable.getPageSize()
					+ " OFFSET " + pageable.getOffset()
					+ " ) adv LEFT JOIN experience exp ON (adv.advId = exp.advId) \r\n"
	...
	public Advisor fetchAdvisorGstByAdvId(String advId, String deleteflag, String encryptPass) {
		try {
			String sql = "SELECT `advId`, `advType`, AES_DECRYPT(`name`,?) name, AES_DECRYPT(`userName`,?) userName, `designation`, AES_DECRYPT(`emailId`,?) emailId, `password`, AES_DECRYPT(`phoneNumber`,?) phoneNumber, `delete_flag`, `partyStatusId`, `created`, `updated`,`created_by`,`updated_by`,`displayName`, AES_DECRYPT(`dob`,?) dob, `gender`, AES_DECRYPT(`panNumber`,?) panNumber, `address1`, `address2`, `state`, `city`, `pincode`, `aboutme`, `firmType`, `parentPartyId`, `corporateLable`, `website`,`gst` FROM `advisor` WHERE `advId` = ? AND `delete_flag`=?";
	...
	public Party fetchParty(long partyId, String encryptPass) {
		try {
			String sql = "SELECT `partyId`,`partyStatusId`,`roleBasedId`,`parentPartyId`,AES_DECRYPT(`emailId`,?) emailId,`password`,AES_DECRYPT(`userName`,?) userName,AES_DECRYPT(`phoneNumber`,?) phoneNumber,AES_DECRYPT(`panNumber`,?) panNumber,`created`,`updated`,`delete_flag`,`created_by`,`updated_by` FROM `party` WHERE `partyId` = ?";
			RowMapper<Party> rowMapper = new BeanPropertyRowMapper<Party>(Party.class);
	...
	public int createBrandsComment(BrandsComment comment, String encryptPass) {
		try {
			ZonedDateTime zdt = ZonedDateTime.now(ZoneId.of("Asia/Calcutta"));
			Timestamp timestamp = Timestamp.valueOf(zdt.toLocalDateTime());
			String sql = "INSERT INTO `brandscomment` (`content`,`partyId`,`parentCommentId`,`name`,`designation`,`imagePath`,`created`,`paramId`,`updated`,`delete_flag`,`created_by`,`updated_by`) values (?,?,?,AES_ENCRYPT(?,?),?,?,?,?,?,?,?,?)";
	...
	// `parentCommentId`,`created`,`partyId`,AES_DECRYPT(`name`,?)
	...
	public int addBlogger(Blogger blog, String deleteflag, String encryptPass) {
		try {
			...
			ZonedDateTime zdt = ZonedDateTime.now(ZoneId.of("Asia/Calcutta"));
			Timestamp timestamp = Timestamp.valueOf(zdt.toLocalDateTime());
			String sql = "INSERT INTO `blogger` (`bloggerId`,`fullName`, `displayName`, `dob`, `gender`, `emailId`, `password`,`userName`, `phoneNumber`, `pincode`, `imagePath`,`partyStatusId`, `created`, `updated`,`delete_flag`,`created_by`,`updated_by`,`blgAdminId`) values (?,AES_ENCRYPT(?,?),?,AES_ENCRYPT(?,?),?,AES_ENCRYPT(LOWER(?),?), ?, AES_ENCRYPT(LOWER(?),?), AES_ENCRYPT(LOWER(?),?), ?,?, ?, ?, ?, ?,?,?,?)";
	...
	public int addPartyBlogger(Blogger blog, String delete_flag_N, String encryptPass) {
		try {
			...
			ZonedDateTime zdt = ZonedDateTime.now(ZoneId.of("Asia/Calcutta"));
			Timestamp timestamp = Timestamp.valueOf(zdt.toLocalDateTime());
			String sqlForPartyInsert = "INSERT INTO `party` (`partyStatusId`,`roleBasedId`,`emailId`,`password`,`userName`,`phoneNumber`,`created`,`updated`,`delete_flag`,`created_by`,`updated_by`) values (?,?,AES_ENCRYPT(LOWER(?),?),?,AES_ENCRYPT(LOWER(?),?), AES_ENCRYPT(LOWER(?),?), ?,?,?,?,?)";
	...
	public Blogger fetchBloggerByBloggerId(String roleBasedId, String deleteflag, String encryptPass) {
		try {
			String sql = "SELECT `bloggerId`, AES_DECRYPT(`fullName`,?) fullName, `displayName`, AES_DECRYPT(`dob`,?) dob, AES_DECRYPT(`emailId`,?) emailId, `gender`,`password`, AES_DECRYPT(`userName`,?)userName ,AES_DECRYPT(`phoneNumber`,?) phoneNumber, AES_DECRYPT(`pincode`,?) pincode,`imagePath`, `partyStatusId`, `created`, `updated`,`delete_flag`,`created_by`,`updated_by`,`blgAdminId` FROM `blogger` WHERE `bloggerId` = ? AND `delete_flag`=?";
	...
	public List<BrandsComment> fetchBrandsComment(String brandId, String delete_flag, String encryptPass) {
		try {
			String sqlBrandInfo = "SELECT commentId, content, partyId, parentCommentId, AES_DECRYPT(name,?) name, designation, imagePath, created,paramId,updated,delete_flag,created_by,updated_by FROM brandscomment WHERE paramId=? AND delete_flag=?";
	...
	public List<Blogger> fetchBlogger(Pageable pageable, String delete_flag, String encryptPass) {
		try {
			String sql = "SELECT `bloggerId`, AES_DECRYPT(`fullName`,?) fullName, `displayName` , AES_DECRYPT(`dob`,?) dob, AES_DECRYPT(`emailId`,?) emailId, `gender`,`imagePath`,`pincode`,`password`, AES_DECRYPT(`phoneNumber`,?) phoneNumber, `delete_flag`, `partyStatusId`, `created`, `updated`,`created_by`,`updated_by` FROM `blogger` WHERE `delete_flag`=?";
			RowMapper<Blogger> rowMapper = new BeanPropertyRowMapper<Blogger>(Blogger.class);
	...
	public int update(String bloggerId, Blogger blogger, String encryptPass) {
		try {
			...
			String sql1 = "UPDATE `blogger` SET `fullName`= AES_ENCRYPT(?, ?),`displayName`=?, `emailId`=AES_ENCRYPT(?, ?), `phoneNumber`=AES_ENCRYPT(?, ?),`userName`=AES_ENCRYPT(?, ?),`dob`=AES_ENCRYPT(?, ?),`gender`=?,`updated`=?,`updated_by`=?,`imagePath`=? WHERE `bloggerId`=?";
	...
	public int updatePersonalInfoInParty(String emailId, String phoneNumber, String userName, String bloggerId,
		...
		try {
			...
			String sql = "UPDATE `party` SET `emailId`=AES_ENCRYPT(?,?),`phoneNumber`=AES_ENCRYPT(?,?),`userName`=AES_ENCRYPT(?,?), `updated`=?,`updated_by`=? WHERE `roleBasedId`=?";
	...
	public List<Advisor> fetchExploreAdvisorListByProduct(String sortByCity, String productId, String serviceId,
		...
		try {
			String sql = "SELECT * FROM (SELECT adv.`advId` advId,adv.`advType` advType,AES_DECRYPT(adv.`name`,?) name,adv.`designation` designation,AES_DECRYPT(adv.`emailId`,?) emailId,adv.`password` password,AES_DECRYPT(adv.`userName`,?) userName,AES_DECRYPT(adv.`phoneNumber`,?) phoneNumber,adv.`delete_flag` delete_flag,adv.`partyStatusId` partyStatusId,adv.`created` created,adv.`updated` updated,adv.`created_by` created_by,adv.`updated_by` updated_by,adv.`displayName` displayName,AES_DECRYPT(adv.`dob`,?) dob,adv.`gender` gender,AES_DECRYPT(adv.`panNumber`,?) panNumber,adv.`address1` address1,adv.`address2` address2,adv.`state` state,adv.`city` city,adv.`pincode` pincode,adv.`aboutme` aboutme,adv.`firmType` firmType,adv.`corporateLable` corporateLable,adv.`website` website,adv.`parentPartyId` parentPartyId,adv.`imagePath` imagePath,adv.`isVerified` isVerified,adv.`verifiedBy` verifiedBy,adv.`verified` verified,adv.`isMobileVerified` isMobileVerified,adv.`workFlowStatus` workFlowStatus,adv.`approvedDate` approvedDate,adv.`approvedBy` approvedBy,adv.`revokedDate` revokedDate,adv.`revokedBy` revokedBy, exp.expId COL_A, exp.company COL_B,  exp.designation COL_C,exp.location COL_D,exp.fromYear COL_E,exp.toYear COL_F,exp.advId COL_ADVID,exp.delete_flag COL_DELETEFLAG, '' COL_I,'' COL_J,'' COL_K,'' COL_L,'' COL_M,'' COL_N, 'exp' VALUE FROM ((SELECT pub_adv.* FROM public_advisor pub_adv INNER JOIN `public_advbrandinfo` pub_brandinfo ON (pub_adv.`advId` = pub_brandinfo.`advId` AND pub_adv.`delete_flag` = ? AND pub_brandinfo.delete_flag=? AND pub_brandinfo.`prodId` ='"
					...
					+ brandId + "') WHERE 1 = (CASE WHEN '" + sortByCity
					+ "' IS NOT NULL THEN 1 ELSE 0 END) AND pub_adv.delete_flag =? AND pub_adv.`city`= '" + sortByCity
					+ "' ORDER BY AES_DECRYPT(pub_adv.`name`,?) ASC LIMIT)) adv LEFT JOIN public_certificate cert ON (adv.advId = cert.advId) \r\n"
	...
	public List<ArticlePost> fetchArticlePostList(long partyId, String deleteflag, String encryptPass) {
		try {
			String sql = "SELECT `articleId`, `created`, `prodId`, `partyId`,AES_DECRYPT(`name`,?) name,`designation`,`imagePath`,`title`, `content`, `forumStatusId`, `updated`, `adminId`, `reason`, `delete_flag`,`created_by`,`updated_by` FROM `articlepost` WHERE `partyId` = ? AND `delete_flag` = ?";
	...
	public List<Advisor> fetchExploreAdvisorByCityList(Pageable pageable, String state, String city, String pincode,
		...
		try {
			String sql = "SELECT `advId`, `advType`, AES_DECRYPT(`name`,?) name,AES_DECRYPT(`userName`,?) userName, `designation`, AES_DECRYPT(`emailId`,?) emailId, `password`, AES_DECRYPT(`phoneNumber`,?) phoneNumber, `delete_flag`, `partyStatusId`, `created`, `updated`, `created_by`, `updated_by`, `displayName`, AES_DECRYPT(`dob`,?) dob, `gender`, AES_DECRYPT(`panNumber`,?) panNumber, `address1`, `address2`, `state`, `city`, `pincode`, `aboutme` FROM `public_advisor` WHERE `state` LIKE '"
	...
	public int updateArticleCommentByPartyId(String displayName, String imagePath, long partyId, String signedUserId,
		...
		try {
			ZonedDateTime zdt = ZonedDateTime.now(ZoneId.of("Asia/Calcutta"));
			Timestamp timestamp = Timestamp.valueOf(zdt.toLocalDateTime());
			String sql = "UPDATE `brandscomment` SET `name`= AES_ENCRYPT(?, ?),`imagepath`=?,`updated`=?,`updated_by`=? WHERE `partyId`=?";
	...
	public List<BrandsComment> fetchBrandsCommentByPartyId(long partyId, String deleteflag, String encryptPass) {
		try {
			String sqlBrandInfo = "SELECT `commentId`, `content`, `partyId`, `parentCommentId`, AES_DECRYPT(`name`,?) `name`, `designation`, `imagePath`, `created`, `paramId`,`updated`,`delete_flag`,`created_by`,`updated_by` FROM `brandscomment` WHERE `partyId`=? AND `delete_flag`=?";
			RowMapper<BrandsComment> rowMapper = new BeanPropertyRowMapper<BrandsComment>(BrandsComment.class);
	...
	public List<Advisor> fetchExploreAdvisorByDisplayName(Pageable pageable, String displayName, String deleteflag,
			String encryptPass, String signedUserId) {
		try {
			String sql = "SELECT `advId`, `advType`, AES_DECRYPT(`name`,?) name,AES_DECRYPT(`userName`,?) userName, `designation`, AES_DECRYPT(`emailId`,?) emailId, `password`, AES_DECRYPT(`phoneNumber`,?) phoneNumber, `delete_flag`, `partyStatusId`, `created`, `updated`, `created_by`, `updated_by`, `displayName`, AES_DECRYPT(`dob`,?) dob, `gender`, AES_DECRYPT(`panNumber`,?) panNumber, `address1`, `address2`, `state`, `city`, `pincode`, `aboutme` FROM `public_advisor` WHERE `displayName` LIKE '"
	...
	public List<BrandsComment> fetchBrandsCommentsList(Pageable pageable, String deleteflag, String encryptPass) {
		try {
			String sql = "SELECT `commentId`,`partyId`,`parentCommentId`,AES_DECRYPT(`name`,?) name, `designation` ,`imagepath`,`content`,`paramId`,`adminId`,`delete_flag`, `created`,			 `updated`,`created_by`,`updated_by` FROM `brandscomment` WHERE			 `delete_flag`=?";
	...
	public int updateArticlePostNameByPartyId(String displayName, String imagePath, long partyId, String signedUserId,
		...
		try {
			ZonedDateTime zdt = ZonedDateTime.now(ZoneId.of("Asia/Calcutta"));
			Timestamp timestamp = Timestamp.valueOf(zdt.toLocalDateTime());
			String sql = "UPDATE `articlepost` SET `name`= AES_ENCRYPT(?, ?),`imagepath`=?,`updated`=?,`updated_by`=? WHERE `partyId`=?";
			int result = jdbcTemplate.update(sql, displayName, encryptPass, imagePath, timestamp, signedUserId,
					partyId);
			return result;
		} catch (DataAccessException e) {
	...
	public List<ArticleComment> fetchArticleCommentByParyId(long partyId, String deleteflag, String encryptPass) {
		try {
			String sql = "SELECT `commentId`,`partyId`, `parentCommentId`,AES_DECRYPT(`name`,?) name,`designation`,`imagePath`, `content`,`created`, `forumStatusId`,`articleId`, `updated`, `adminId`, `delete_flag`,`created_by`,`updated_by` FROM `articlecomment` WHERE `partyId` = ? AND `delete_flag` = ?";
	...
	public int updateArticlePostCommentByPartyId(String displayName, String imagePath, long partyId,
		...
		try {
			ZonedDateTime zdt = ZonedDateTime.now(ZoneId.of("Asia/Calcutta"));
			Timestamp timestamp = Timestamp.valueOf(zdt.toLocalDateTime());
			String sql = "UPDATE `articlecomment` SET `name`= AES_ENCRYPT(?, ?),`imagepath`=?,`updated`=?,`updated_by`=? WHERE `partyId`=?";
			int result = jdbcTemplate.update(sql, displayName, encryptPass, imagePath, timestamp, signedUserId,
					partyId);
			return result;
		} catch (DataAccessException e) {
